...
export function filterTickets(tickets: Ticket[], filters: TicketFilters): Ticket[] {
  return tickets.filter((ticket) => {
    if (filters.airline && filters.airline !== "all" && ticket.airlineCode !== filters.airline) {
      return false;
    }
    if (filters.sector && filters.sector !== "all" && ticket.sector !== filters.sector) {
      return false;
    }
    if (filters.fromCity && filters.fromCity !== "all" && ticket.fromCity !== filters.fromCity) {
      return false;
    }
    if (filters.toCity && filters.toCity !== "all" && ticket.toCity !== filters.toCity) {
      return false;
    }
    if (filters.destination && filters.destination !== "all" && ticket.destination !== filters.destination) {
      return false;
    }
    if (filters.date && ticket.date !== filters.date) {
      return false;
    }
    if (filters.maxPrice !== undefined && ticket.price > filters.maxPrice) {
      return false;
    }
    if (filters.minSeats !== undefined && ticket.seatsLeft < filters.minSeats) {
      return false;
    }
    return true;
  });
